...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
    ...
    if ((*pgdir_entry & PTE_V) == 0) {
        if (create) {
            *pgdir_entry = PADDR(alloc(BY2PG, BY2PG, 1)) | PTE_V;
        } else return 0;
    }
...
void page_init(void)
{
	...
	struct Page* pageTmp = pages;
	for (; page2kva(pageTmp) < freemem; pageTmp++) {
		pageTmp -> pp_ref = 1;
	}

	/* Step 4: Mark the other memory as free. */
	for(pageTmp = &pages[PPN(PADDR(freemem))]; page2ppn(pageTmp) < npage; pageTmp++) {
		if (page2kva(pageTmp) >= ROUNDDOWN(TIMESTACK-sizeof(struct Trapframe), BY2PG) && page2kva(pageTmp) < TIMESTACK) {
            continue;   //lab3-1
        }
		pageTmp -> pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, pageTmp, pp_link);
	}
}
