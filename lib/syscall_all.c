...
int sys_set_env_status(int sysno, u_int envid, u_int status)
{
	...
	/*
	u_int origin_status = env -> env_status;

	if (status == ENV_RUNNABLE) {
		env -> env_status = status;
		if (origin_status != ENV_RUNNABLE) {
			LIST_INSERT_TAIL(&env_sched_list[0], env, env_sched_link);
		}
	} else if (status == ENV_NOT_RUNNABLE || status == ENV_FREE) {
		env -> env_status = status;
		if (origin_status == ENV_RUNNABLE) {
...
int sys_write_dev(int sysno, u_int va, u_int dev, u_int len)
{
	...
	u_int start_addr[3] = {0x10000000, 0x13000000, 0x15000000};
	u_int end_addr[3] = {0x10000020, 0x13004200, 0x15000200};
	int i;
	int check = 0;
	for (i = 0; i < 3; i++) {
		if (dev >= start_addr[i] && (dev + len <= end_addr[i])) {
			check = 1;
			break;
		}
	}
